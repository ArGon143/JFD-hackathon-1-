...
function Notification() {
  ...
  this.addItem = function (notification) {
    ...
    notification.prev =
      ...
      this.elements[this.elements.length - this.notificationCount - 1];
    ...
    animateAppearing.call(notification.item).then(() => {
  ...
  function removeItem(wrapper, notification) {
    animateDisappearing.call(notification.item).then(() => {
      setTimeout(() => {
        try {
        ...
        wrapper.elements = wrapper.elements.filter(
          (e) => e !== notification.item
        );
      }, 1000);
    });
  }
