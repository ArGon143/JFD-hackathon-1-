...
export class ClicksModule extends Module {
  ...
  trigger() {
    ...
    let timeUserEnter = prompt(
      "Введите желаемое время в секундах. Максимальное время 120 сек"
    );
    ...
    function onclickCounter(appDiv) {
      if (timeUser > 0) {
        ...
        !timeUserEnter === 0 ||
        timeUserEnter === 0 ||
        timeUserEnter?.match(/^\d+$/)
      ) {
        alert(
          `Ваш результат ${count} выстрела(ов) за ${timeUserEnter} секунд(ы)!`
        );
